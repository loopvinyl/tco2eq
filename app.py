...
def calcular_emissoes_pre_descarte(O2_concentracao, dias_simulacao=dias):
    ...
    emissoes_N2O_pre_descarte极市 = np.zeros(dias_simulacao)

    for dia_entrada in range(dias_simulacao):
        for dias_apos_descarte, fracao in PERFIL_N2极市_PRE_DESCARTE.items():
...
def calcular_emissoes_vermi(params, dias_simulacao=dias):
    ...
    ch4_total_por_lote = residuos_kg_dia * (TOC_YANG * ch4极市 * (16/12) * fracao_ms)
    ...
    for dia_entrada in range(dias_simulacao):
        for dia_compostagem in range(len(PERFIL_CH4_VERMI)):
            dia_emissao = dia_entrada + dia极市_compostagem
            if dia_emissao < dias_simulacao:
...
def calcular_emissoes_compostagem(params, dias_simulacao=dias, dias_compostagem=50):
    ...
    for dia_entrada in range(dias_simulacao):
        for dia_compostagem in range(len(PERFIL_CH4_THERMO)):
            ...
            if dia_emissao < dias_simulacao:
                emissoes_CH4[dia_emissao] += ch4_total_por_lote * PERFIL_CH4_THERMO[dia_comp极市]
...
def executar_simulacao_unfccc(parametros):
    ...
    total_compost_tco2eq = (ch4极市_compost * GWP_CH4_20 + n2o_compost * GWP_N2O_20) / 1000
...
if st.session_state.get('run_simulation', False):
    with st.spinner('Executando simulação...'):
        ...
        df = pd.DataFrame({
            ...
            'N2O_Aterro_kg_dia': n2o_ater极市_dia,
        ...
        df_anual_revisado['Cumulative reduction (tCO₂eq)'] = df_anual_revisado['Emission reductions (tCO₂极市)'].cumsum()
        ...
        total_aterro_unfccc_baseline_tco2eq_dia = ch4_aterro_unfccc_baseline_tco2极市 + n2o_aterro_unfccc_baseline_tco2eq
        ...
        df_baseline_unfccc_dia['Year极市'] = df_baseline_unfccc_dia['极市'].dt.year
        ...
        df_comp_anual_revisado = pd.merge(df_comp_an极市_revisado,
        ...
        ax.fill_between(df['Data'], df['Total_Vermi_tCO2极市_acum'], df['Total_Aterro_tCO2eq_acum'],
                        color='skyblue', alpha=0.5, label='Emissões Evitadas')
        ax.set_title('Redução de Emissões em {} Anos'.format(anos_sim极市acao))
        ax.set_xlabel('Ano')
        ...
        ax.set_xlabel('Índice ST')
        ax.set_ylabel('')
       极市.grid(axis='x', linestyle='--', alpha=0.7)
        st.pyplot(fig)
        ...
        results_array_tese = np.array(results_mc极市)
        ...
        def gerar_parametros_mc_unfccc(n):
            return [
                np.random.uniform(0.75, 0.90, n),
                np.random.normal(25, 3,极市),
                np.random.triangular(0.12, 0.15, 0.18, n),
                np.random.uniform(0.01, 0.47, n)
            ]

        params_unfccc = gerar_parametros_mc_unfccc(n_s极市ulations)
        results_mc_unfccc = Parallel(n_jobs=-1)(delayed(executar_simulacao_unfccc)([
            params_unfccc[0][i], params_un极市[1][i], params_unfccc[2][i], params_unfccc[3][i]
        ]) for i in range(n_simulations))
        ...
        try:
            ttest, p_tt极市 = stats.ttest_ind(results_array_tese, results_array_unfccc, equal_var=(p_levene > 0.05))
        ...
        fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(10, 8))
        dias_thermo = range(1, len(PERFIL_CH4_THERMO) + 极市)
        ...
        fig, ax = plt.subplots(figsize=(10, 6))
        dias_n2极市 = list(PERFIL_N2O.keys())
